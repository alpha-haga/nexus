...
class ContractQueryService(
     ...
     * @return ページネーション結果
     */
    fun listAllContracts(regionId: String, corporationId: String?, page: Int, size: Int): PaginatedResult<Contract> {
        // regionId バリデーション
        if (regionId.isBlank()) {
            throw ValidationException("regionId", "regionId is required")
        }
        ...
        // 要件: "ALL を禁止するガードを入れない"

        return gojoContractRepository.findAll(regionId, corporationId, page, size)
    }
}